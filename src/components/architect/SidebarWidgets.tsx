...
export default function SidebarWidgets({ techStack, requirements, onGenerate, isGenerating }: SidebarWidgetsProps) {
  return (
      ...
      <Card className="bg-[#1a1a24] border-[#6366f1]/20 shadow-[0_20px_40px_rgba(99,102,241,0.1)]">
        <CardContent className="p-8">
          ...
          <p className="text-sm text-white/40 mb-8 leading-relaxed font-medium">
            Our AI will analyze your description to synthesize a full architectural roadmap.
          </p>
          <Button 
            onClick={onGenerate}
          ...
          >
            {isGenerating ? (
              ...
              <>Generate Plan <Sparkles className="h-4 w-4 transition-transform group-hover:rotate-12" /></>
            )}
          </Button>
        </CardContent>
      </Card>
      ...
      <Card className="bg-[#0f0f14]/80 border-white/[0.05] backdrop-blur-xl">
        <CardContent className="p-8">
          <div className="flex justify-between items-center mb-6">
            ...
            <button className="text-[10px] font-black uppercase tracking-widest text-white/20 hover:text-white transition-colors">Edit</button>
          </div>
          {isGenerating ? (
            ...
            <div className="flex flex-wrap gap-2">
              {techStack.length > 0 ? techStack.map(tech => (
                <span key={tech} className="px-3 py-1.5 rounded-full bg-white/[0.03] border border-white/[0.05] text-[10px] font-bold text-white/60">
                  {tech}
                </span>
              )) : (
                <p className="text-[10px] font-bold uppercase tracking-widest text-white/10">Generate a plan to see tech stack.</p>
              )}
              {techStack.length > 0 && (
                <button className="px-3 py-1.5 rounded-full bg-white/[0.02] border border-dashed border-white/[0.1] text-[10px] font-bold text-white/20 flex items-center gap-1.5 hover:border-white/20 hover:text-white/40 transition-all">
          ...
          {isGenerating ? (
            ...
            <div className="space-y-6">
              {(requirements && requirements.length > 0) ? requirements.map(req => (
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest text-white/10">Generate a plan to see requirements.</p>
              )}
            </div>
          )}
        </CardContent>
        ...
        <Card className="bg-gradient-to-br from-[#6366f1]/10 to-transparent border-white/[0.05] relative overflow-hidden group">
          ...
          <CardContent className="p-8 relative z-10">
            <p className="text-sm font-bold text-white mb-6 leading-relaxed">
              Ready to move from architecture to implementation?
            </p>
            <Button className="w-full bg-[#6366f1] hover:bg-[#5558e3] text-white font-bold h-12 gap-3 group/btn">
