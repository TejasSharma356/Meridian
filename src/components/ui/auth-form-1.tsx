...
import { signInUser, signUpUser, googleSignInUser } from "@/app/auth/actions";
...
function AuthSocialButtons({ isLoading }: AuthSocialButtonsProps) {
  ...
  const login = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        const res = await googleSignInUser(tokenResponse.access_token);
        if (res?.error) {
          toast.error(res.error);
        } else {
          toast.success('Google Access granted. Booting system...');
          router.push(res.redirectTo || '/initialize');
        }
      } catch (err) {
    ...
    onError: () => {
      toast.error("Google authentication popup failed");
    }
  });
  ...
  return (
    <div data-slot="auth-social-buttons" className="w-full mt-6">
      <Button
        type="button"
        variant="outline"
        className="w-full h-12 bg-background/50 border-border/50 text-white"
        disabled={isLoading}
        onClick={() => login()}
      >
...
function AuthSignIn({ onForgotPassword, onSignUp }: AuthSignInProps) {
  ...
  const onSubmit = async (data: SignInFormValues) => {
    ...
    try {
      ...
      if (res?.error) {
        setFormState((prev) => ({ ...prev, error: res.error }));
      } else {
  ...
  return (
    ...
    >
      ...
      <AuthForm onSubmit={handleSubmit(onSubmit)}>
        ...
        <Button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-500 text-white" disabled={formState.isLoading}>
          {formState.isLoading ? (
            <>
      ...
      <p className="mt-8 text-center text-sm text-muted-foreground">
        ...
        <Button
          variant="link"
          className="h-auto p-0 text-sm text-indigo-400"
          onClick={onSignUp}
          disabled={formState.isLoading}
        >
          Create one
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              ...
              <Button variant="link" className="h-auto p-0 text-xs text-indigo-400">
                Terms
              </Button>{" "}
              and{" "}
              <Button variant="link" className="h-auto p-0 text-xs text-indigo-400">
